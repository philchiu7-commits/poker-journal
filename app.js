...
const HUD_DRILL_LABEL = {
  ...
  bxtF: ["Bet when checked to", "Checked back"],
  bxtT: ["Bet when checked to", "Checked back"],
  bxtR: ["Bet when checked to", "Checked back"],
...
const STAT_DRILL = {
  ...
  probeTHu: { ev: ["probeTHu"],                     yes: "Probed the turn",   no: "Checked it to him" },
  probeTMw: { ev: ["probeTMw"],                     yes: "Probed the turn",   no: "Checked it to him" },
...
function rangePosGroups(o, rowsBy) {
  ...
  return [RANGE_ANY].concat(POSITIONS.filter((p) => seats.has(p)).map(rangePosGroup));
}
